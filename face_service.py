...
Pipeline for attendance check-in:
  1. extract_embedding()  — detect face, run liveness check, return embedding
  2. find_best_match()    — compare embedding against stored members
"""
...
_face_app    = None
_spoof_sess  = None
_face_lock   = threading.Lock()   # prevents race condition during init
...
def _check_liveness(img_bgr, bbox) -> tuple:
    """
    Run anti-spoof check on the face crop.
    Returns (is_real: bool, score: float)
    """
    sess = _get_spoof_session()
    if sess is None:
        return True, 1.0  # no model — skip check
...
def extract_embedding(image_bytes: bytes) -> dict:
    """
    Full pipeline: detect face → liveness check → extract embedding.
...
_spoof_sess = None   # ONNX anti-spoof session

# Path to the anti-spoof ONNX model
_ANTISPOOF_MODEL = os.path.join(
    os.path.dirname(__file__), 'static', 'antispoof', 'antispoof.onnx'
)

# Liveness threshold — score > this = real face
# Lower = stricter (fewer false accepts), higher = more lenient (fewer false rejects)
LIVENESS_THRESHOLD = 0.6


def _get_face_app():
    """Load InsightFace once, return None if unavailable."""
    if not INSIGHTFACE_AVAILABLE:
        return None
    global _face_app
    if _face_app is None:
...
def _get_spoof_session():
    """Load anti-spoof ONNX session once, return None if unavailable."""
    if not ONNX_AVAILABLE:
        return None
    if not os.path.exists(_ANTISPOOF_MODEL):
        return None
    global _spoof_sess
    if _spoof_sess is None:
        opts = ort.SessionOptions()
...
def _check_liveness(img_bgr, bbox) -> tuple[bool, float]:
    """
    Run anti-spoof check on the face crop.

    Args:
        img_bgr: full frame as numpy BGR array
        bbox:    [x1, y1, x2, y2] face bounding box from InsightFace

    Returns:
        (is_real, score) — score is probability of being a real face (0-1)
    """
    sess = _get_spoof_session()
    if sess is None:
        # No model available — skip liveness check (allow all)
        return True, 1.0
    ...
    # Add padding around face (20% each side) for better context
    h, w = img_bgr.shape[:2]
    pad_x = int((x2 - x1) * 0.2)
    pad_y = int((y2 - y1) * 0.2)
    x1 = max(0, x1 - pad_x)
    ...
    face_rgb = cv2.cvtColor(face_resized, cv2.COLOR_BGR2RGB).astype(np.float32) / 255.0
    face_tensor = np.transpose(face_rgb, (2, 0, 1))[np.newaxis, :]  # (1, 3, 128, 128)

    # Run inference
    input_name = sess.get_inputs()[0].name
    outputs = sess.run(None, {input_name: face_tensor})
    logits = outputs[0][0]  # shape: (2,) — [spoof_score, real_score]
...
def extract_embedding(image_bytes: bytes) -> dict | None:
    """
    Full pipeline: detect face → liveness check → extract embedding.

    Returns a dict:
        {'embedding': [...], 'liveness': True, 'liveness_score': 0.95}
    or on failure:
        None                          — no face detected
        {'liveness': False, ...}      — spoof detected
    Returns None if InsightFace is unavailable.
    """
    app = _get_face_app()
    if app is None:
        return None
